...
export const MapSection: React.FC = () => {
  const [activeLocation, setActiveLocation] = useState<'marylebone' | 'mayfair'>('marylebone');

  const locations = {
    marylebone: {
      name: 'Marylebone Flagship',
      address: '42 George Street, Marylebone, London W1U 7EY',
      phone: '+44 (0)20 7486 9000',
      hours: 'Mon - Sat: 9:00 AM - 9:00 PM',
      mapCoords: { lat: 51.5175, lng: -0.1558 },
    },
    mayfair: {
      name: 'Mayfair Studio',
      address: '18 Mount Street, Mayfair, London W1K 2RH',
      phone: '+44 (0)20 7629 1111',
      hours: 'Mon - Sat: 10:00 AM - 8:00 PM',
      mapCoords: { lat: 51.5095, lng: -0.1492 },
    }
  };
    ...
    <section className="py-16 bg-white dark:bg-zinc-950 border-t border-zinc-200 dark:border-zinc-800 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            <span className="text-xs font-bold uppercase tracking-widest text-zinc-400">LONDON LOCATIONS</span>
          ...
          <div className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-900 p-1.5 rounded-full border border-zinc-200 dark:border-zinc-800">
            ...
            <button
              onClick={() => setActiveLocation('mayfair')}
              className={`text-xs font-bold uppercase tracking-wider px-5 py-2.5 rounded-full transition-all cursor-pointer ${
                activeLocation === 'mayfair'
                  ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-950 shadow-md'
                  : 'text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white'
              }`}
            >
        ...
        <div className="relative rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 shadow-xl min-h-[420px] bg-zinc-100 dark:bg-zinc-900 flex flex-col justify-between">
          ...
          {/* Stylized London Map Canvas (matches screenshot layout with grid lines & street names) */}
          ...
          {/* SVG London City Map Mockup with interactive location marker */}
          <div className="absolute inset-0 overflow-hidden flex items-center justify-center">
            <svg className="w-full h-full text-zinc-300 dark:text-zinc-800" fill="none" stroke="currentColor" strokeWidth="1.5">
              {/* Thames River curve */}
              <path d="M 0 350 Q 300 320 600 380 T 1200 420" stroke="#93c5fd" strokeWidth="18" fill="none" opacity="0.5" />
              {/* Street grid paths */}
          ...
          {/* Floating Location Info Box (matching bottom left overlay) */}
